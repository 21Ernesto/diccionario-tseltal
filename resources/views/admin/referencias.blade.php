    ...
    <main class="p-4 md:ml-64 h-auto pt-20">
            ...
            <ol class="flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-xl font-semibold text-gray-400 md:ml-2">Referencias</span>
                    </div>
                </li>
            </ol>
            ...
            <table id="miTabla" class="w-full text-sm text-left">
                <thead class="text-xs text-gray-100 uppercase bg-gray-700 text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">pub</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <div class="mt-4">
            <ul class="flex items-center justify-end space-x-2">
                ...
                @else
                    <li>
                        <a href="{{ $referencia->previousPageUrl() }}"
                ...
                @for ($i = max($referencia->currentPage() - 2, 1); $i <= min($referencia->currentPage() + 1, $referencia->lastPage()); $i++)
                    <li>
                        <a href="{{ $referencia->url($i) }}"
                            class="pagination-item font-medium py-1.5 px-4 bg-gray-700 rounded text-gray-100">{{ $i }}</a>
                    </li>
                @endfor

                @if ($referencia->hasMorePages())
                    <li>
                        <a href="{{ $referencia->nextPageUrl() }}"
                            class="relative inline-flex items-center px-4 py-1.5 text-sm font-medium text-gray-100 bg-gray-700 cursor-pointer rounded">
                            Adelante
                        </a>
                    </li>
        ...
        <div id="abecedarioModal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-2xl max-h-full">
                <div class="relative rounded-lg shadow bg-gray-700">
                    ...
                    <form method="POST" action="{{ route('referencia.import') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="p-6">
