    ...
    <main class="p-4 md:ml-64 h-auto pt-20">
            ...
            <ol class="flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-xl font-semibold text-gray-400 md:ml-2">Abecedario</span>
                    </div>
                </li>
            </ol>
            ...
            <table id="miTabla" class="w-full text-sm text-left">
                <thead class="text-xs  uppercase bg-gray-50 bg-gray-700 text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">Comilla</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <div class="mt-4">
            <ul class="flex items-center justify-end space-x-2">
                @if ($abecedario->onFirstPage())
                    <li
                        class="relative inline-flex items-center px-4 py-1.5 text-sm font-medium  text-white bg-gray-700 cursor-not-allowed rounded">
                        Atras
                    </li>
                @else
                    <li>
                        <a href="{{ $abecedario->previousPageUrl() }}"
                            class="relative inline-flex items-center px-4 py-1.5 text-sm font-medium  bg-gray-700 cursor-pointer rounded">
                            Atras
                        </a>
                    </li>
                @endif

                @for ($i = max($abecedario->currentPage() - 2, 1); $i <= min($abecedario->currentPage() + 1, $abecedario->lastPage()); $i++)
                    <li>
                        <a href="{{ $abecedario->url($i) }}"
                            class="pagination-item font-medium py-1.5 px-4 bg-gray-700 rounded ">{{ $i }}</a>
                    </li>
                @endfor

                @if ($abecedario->hasMorePages())
                    <li>
                        <a href="{{ $abecedario->nextPageUrl() }}"
                            class="relative inline-flex items-center px-4 py-1.5 text-sm font-medium  bg-gray-700 cursor-pointer rounded">
                            Adelante
                        </a>
                    </li>
                @else
                    <li
                        class="relative inline-flex items-center px-4 py-1.5 text-sm font-medium  bg-gray-700 cursor-not-allowed rounded">
                        Adelante
                    </li>
                @endif
            </ul>
        ...
        <div id="abecedarioModal" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative w-full max-w-2xl max-h-full">
                ...
                <div class="relative rounded-lg shadow bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form method="POST" action="{{ route('abecedario.import') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="p-6">
